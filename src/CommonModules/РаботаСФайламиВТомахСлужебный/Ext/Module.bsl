	...
	Иначе
		...
		Если ТомХранения = Неопределено Тогда
			...
			НСтр("ru = 'Не удалось добавить файл ""%1"" ни в один из томов, т.к. в томах недостаточно места.';
				|en = 'Cannot add file ""%1"" to any of the volumes as they do not have enough space.'"),
				ПрисоединенныйФайл.Наименование + "." + ПрисоединенныйФайл.Расширение);
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Файлы.Удаление файлов в томе';
					|en = 'Files.Delete files from volume'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОчиститьУдаленныеФайлыВТоме(Каталог)
	...
	Для Каждого Файл Из УдаленныеФайлы(Каталог.Тома, КорневойКаталог, ФайлыДляУдаления) Цикл
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Работа с файлами.Очистка файлов';
			...
			,
			НСтр("ru = 'Удален файл:';
				|en = 'File deleted:'") + Символы.НПП + Файл);
		УдалитьФайл(Файл);
	КонецЦикла;
...
Функция ПредставлениеСтатусаПроверки(Знач СтатусПроверки) Экспорт
	...
	ИначеЕсли СтатусПроверки = "ЛишнийФайлВТоме" Тогда 
		Возврат НСтр("ru = 'Лишние файлы (есть на диске, но сведения о них отсутствуют)';
					|en = 'Extraneous files (files on the hard drive that are not registered in the application)'");
	ИначеЕсли СтатусПроверки = "НетФайлаВТоме" Тогда
		Возврат НСтр("ru = 'Отсутствуют файлы в томе на диске';
					|en = 'No files in volume on hard drive'");
	КонецЕсли;
КонецФункции
...
Функция СвободныйТом(РазмерФайла)
	...
	Если Результат.Пустой() Тогда
		ВызватьИсключение НСтр("ru = 'Нет ни одного тома для размещения файла.';
								|en = 'There is no volume available for storing the file.'");
	КонецЕсли;
