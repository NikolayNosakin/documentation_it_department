...
Процедура Инициализировать(КомпонентыОбмена, Анализ = Ложь) Экспорт
	...
	ШаблонНаименования = НСтр("ru = '%1 в %2 для ""%3"" (%4)';
								|en = '%1 on %2 for %3 (%4)'", КодОсновногоЯзыка); // Отправка в 18.07.2022 15:09 для "Бухгалтерия предприятия 3.0"
...
Процедура Завершить(КомпонентыОбмена) Экспорт
	...
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = '--- Итоговые значения по событиям ---';
									|en = '--- Totals by event ---'", КодОсновногоЯзыка));
	ЗаписьИтогов.ЗаписатьСтроку(НСтр("ru = 'Процент; Ср. время; Время; Кол-во событий; Тип события; Событие';
									|en = 'Percentage; Average time; Time; Number of events; Event type; Event'", КодОсновногоЯзыка));
...
Функция ТипСобытияПрикладное() Экспорт
	...
	Возврат НСтр("ru = 'Остальная конфигурация';
				|en = 'Other configuration components'", ОбщегоНазначения.КодОсновногоЯзыка());
