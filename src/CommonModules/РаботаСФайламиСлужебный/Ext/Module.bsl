...
Процедура ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов(ВозможныеПрава) Экспорт
	...
	Право.Подсказка     = НСтр("ru = 'Добавление, изменение и
	                                 ...
	                                 |en = 'Add, edit, and mark folders for deletion.'");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий =
			НСтр("ru = 'Перенос информации о наличии файлов в регистр сведений Наличие файлов.';
				|en = 'Move file availability data to the ""Has files"" information register.'");
...
Функция ИменаСправочниковХраненияВерсийФайлов(ВладелецФайлов, НеВызыватьИсключение = Ложь)
	...
	Ошибки.Добавить(НСтр("ru = 'Ошибка при определении имен справочников для хранения версий файлов.';
						|en = 'An error occurred when determining names of file version storage catalogs.'"));
	ОсновнойСправочникУказан = Ложь;
...
Функция ОбновитьВерсиюФайла(ФайлСсылка,
	...
	Если СведенияОФайле.ХранитьВерсии Тогда
		...
		ЗаголовокОшибки = НСтр("ru = 'Ошибка при записи новой версии присоединенных файлов.';
								|en = 'An error occurred when saving a new version of the attached files.'");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УдалениеПомеченныхОбъектов") Тогда
		...
		Если Не РезультатУдаления.Успешно Тогда
			ЗаписатьВЖурналСобытийОчисткиФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|en = 'Not all unwanted files were deleted automatically (%1) as they are being used in other parts of the application or due to other reasons.
				|To view the reasons why the files cannot be deleted, open ""Marked objects deletion"" in the application settings.'"),
...
Процедура СинхронизироватьФайлыСОблачнымСервисом(УчетнаяЗапись)
	...
	ТекстСобытия = НСтр("ru = 'Завершена синхронизация файлов с облачным сервисом';
						|en = 'File synchronization with cloud service completed'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
...
Функция СинхронизироватьФайлы(ТаблицаФайлов, ПараметрыСинхронизации, АдресСервера, ЭтоУдалениеФайлов = Ложь)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
		...
		Исключение
			...
			СинхронизацияВыполнена = Ложь;
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось синхронизировать файл ""%1"" по причине:';
...
Процедура ВыполнитьПроверкуПодключения(УчетнаяЗапись, РезультатПроверки) Экспорт 
	...
	ТекстСобытия = НСтр("ru = 'Начата проверка синхронизации файлов';
						|en = 'File synchronization check started'") + " " + УчетнаяЗапись.Наименование;
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
	...
	ТекстСобытия = НСтр("ru = 'Завершена проверка синхронизации файлов';
						|en = 'File synchronization check completed'") + " " + УчетнаяЗапись.Наименование;
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
...
Процедура УдалитьНесинхронизируемыеФайлыУчетнойЗаписи(УчетнаяЗапись = Неопределено)
	...
	ТекстСобытия = НСтр("ru = 'Завершено освобождения файлов, захваченных облачным сервисом';
						|en = 'Releasing files locked by the cloud service completed'");
	ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись);
...
Процедура ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведенийДляТаблицы(ПараметрыОбновления, ПолноеИмяОбъектаМетаданных, ОбработкаЗавершена)
	...
	Если ОбъектМетаданных = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не указан объект для обработки электронных подписей и сертификатов шифрования.';
								|en = 'An object for processing digital signatures and encryption certificates is not specified.'");
	КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать объект %1 по причине:
			           |%2';
			           |en = 'Couldn''t process object %1. Reason:
			           |%2'"),
			ОбъектМетаданных,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать некоторые объекты (пропущены): %1';
				|en = 'Couldn''t process (skipped) some objects: %1'"),
			ПроблемныхОбъектов);
		...
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обработана очередная порция объектов: %1';
					|en = 'Yet another batch of objects is processed: %1'"),
				ОбъектовОбработано));
	КонецЕсли;
