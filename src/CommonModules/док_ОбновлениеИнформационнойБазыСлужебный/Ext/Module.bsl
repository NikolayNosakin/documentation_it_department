...
Процедура ПерезаполнитьКластер1С() Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Запрос = Новый запрос("ВЫБРАТЬ
	|	КлПлатформы.Ссылка КАК Ссылка,
	|	КлПлатформы.НомерСтроки КАК НомерСтроки,
	|	КлПлатформы.Версия КАК Версия,
	|	КлПлатформы.Разрядность КАК Разрядность,
	|	КлПлатформы.ДиапазонПортов КАК ДиапазонПортов,
	|	КлПлатформы.Логин КАК Логин,
	|	КлПлатформы.Пароль КАК Пароль,
	|	КлПлатформы.Ссылка.Сервер КАК Сервер
	|ИЗ
	|	Справочник.док_Кластеры1С.Платформы КАК КлПлатформы");
	ТабКластеры = Запрос.Выполнить().Выгрузить();
	Для каждого стрКл из ТабКластеры Цикл
		Если стрКл.НомерСтроки = 1 Тогда
			ОбКл = стрКл.Ссылка.ПолучитьОбъект();
		Иначе
			ОбКл = Справочники.док_Кластеры1С.СоздатьЭлемент();
			ОбКл.Сервер = стрКл.Сервер;		
		КонецЕсли; 
		ОбКл.Версия = стрКл.Версия;
		ОбКл.Разрядность = ?(стрКл.Разрядность = Перечисления.док_Разрядность.x32_x64,Перечисления.док_Разрядность.x64,стрКл.Разрядность);
		ОбКл.Порт = стрКл.ДиапазонПортов;
		ОбКл.Логин = стрКл.Логин;
		ОбКл.Пароль = стрКл.Пароль;
		ОбКл.Записать();
	КонецЦикла;
