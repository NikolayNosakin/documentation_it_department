...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Обработчик.Комментарий = НСтр("ru = 'Перенос настроек входа пользователей';
										|en = 'User authentication settings migration'");
		...
		Если ЕстьРазделАдминистрирование Тогда
			Подсказка =
				НСтр("ru = 'Требования к длине и сложности пароля задаются отдельно.
				           ...
				           |en = 'You can apply password length and complexity requirements.
				           |See ""Administration"" – ""Users and rights settings""
				           |<a href = ""%1"">External user authorization settings</a>.'");
		Иначе
			Подсказка =
				НСтр("ru = 'Требования к длине и сложности пароля задаются отдельно.
				           |См. <a href = ""%1"">Настройки входа внешних пользователей</a>.';
				           |en = 'You can apply password length and complexity requirements.
				           |See <a href = ""%1"">External user authorization settings</a>.'");
		КонецЕсли;
	Иначе
		Если ЕстьРазделАдминистрирование Тогда
			Подсказка =
				НСтр("ru = 'Требования к длине и сложности пароля задаются отдельно.
				           ...
				           |en = 'You can apply password length and complexity requirements.
				           |See ""Administration"" – ""Users and rights settings""
				           |<a href = ""%1"">Users authorization settings</a>.'");
		Иначе
			Подсказка =
				НСтр("ru = 'Требования к длине и сложности пароля задаются отдельно.
				           |См. <a href = ""%1"">Настройки входа пользователей</a>.';
				           |en = 'You can apply password length and complexity requirements.
				           |See <a href = ""%1"">Users authorization settings</a>.'");
		КонецЕсли;
	КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать пользователя %1 по причине: %2';
				|en = 'Couldn''t process the %1 user due to: %2'"),
...
Функция ИмяСобытияОшибкаВходаВПрограммуДляЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Пользователи.Ошибка входа в программу';
...
Функция ОшибкаДлиныИлиСложностиПароля(Пароль, Знач МинимальнаяДлинаПароля, СложныйПароль, ИмяДляВхода) Экспорт
	...
	Если СложныйПароль И СтрНайти(ВРег(Пароль), ВРег(ИмяДляВхода)) Тогда
		Возврат НСтр("ru = 'Пароль содержит Имя (для входа).';
					|en = 'The password contains the username.'")
			+ Символы.ПС + Символы.ПС
			+ ПодсказкаДляНовогоПароля();
	КонецЕсли;
