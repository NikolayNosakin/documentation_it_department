			...
			Исключение
				...
				ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, 
					НСтр("ru = 'В процессе выборки пользовательских настроек отчетов из системного хранилища возникла ошибка:';
						|en = 'Error selecting custom report setting from the system storage:'")
					+ Символы.ПС
					+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		...
		Исключение
			...
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru = 'В процессе выборки вариантов отчетов из стандартного хранилища возникла ошибка:';
					|en = 'Error occurred selecting report options from the standard storage:'")
				+ Символы.ПС
				+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
			...
			Если Результат.СсылкаВарианта = Неопределено Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В методе ""%1"" не указаны параметры ""%2"".';
						|en = 'Parameters %2 are not provided in method %1.'"),
			...
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отчет ""%1"" не существует в программе, он будет значиться как внешний.';
						|en = 'The ""%1"" report is not a part of the application. It is registered as an external report.'"),
					ОтчетПолноеИмя));
...
Функция ХранилищеВариантовДополнительногоОтчетаКорректное(МетаданныеОтчета, ТекстПредупреждения = "") Экспорт 
	...
	ШаблонПредупреждения = НСтр("ru = 'Свойство отчета ""Хранилище вариантов"" %1.
		...
		|Please contact the report developer.'");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка источника данных в настройках вариантов универсального отчета.
		...
		|en = 'Configuring data source in universal report option settings.
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("Строка") Тогда
		Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отчет ""%1"" записан как внешний и не может быть подключен из программы';
				|en = 'Cannot attach report %1 from the application. Reason: the report is saved as external report.'"),
			Результат.Ссылка);
		Возврат Результат;
	КонецЕсли;
		...
		Если ТипЗнч(Результат.Метаданные) <> Тип("ОбъектМетаданных") Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отчет ""%1"" не существует в программе';
					|en = 'The ""%1"" report is not a part of the application.'"),
				Результат.Имя);
			Возврат Результат;
		КонецЕсли;
	...
	Для Каждого Изменение Из Изменения Цикл
		...
		Если Найденные.Количество() > 2 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|The previous name also registered as
				|previous name %4 (current name %5).'"),
				Строка(Изменение.Отчет),
				Изменение.СтароеИмяВарианта,
				Изменение.АктуальноеИмяВарианта,
...
Процедура ЗаменитьКлючиПользовательскихНастроек(СтарыйВариант, АктуальныйВариант)
	...
	Пока Истина Цикл
		...
		Исключение
			...
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru = 'В процессе выборки вариантов отчетов из стандартного хранилища возникла ошибка:';
					|en = 'Error selecting report options from standard storage:'")
...
Функция ПользовательскиеНастройкиВариантаОтчета(ВариантОтчета, Пользователь, КлючНастройки = Неопределено)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Чтение пользовательских настроек отчета';
				|en = 'Read user settings of report'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,
...
Функция СообщениеДанныеЕщеОбновляются() Экспорт
	Возврат НСтр("ru = 'Отчет может содержать некорректные данные, так как не завершен переход на новую версию программы. Если отчет долгое время недоступен, обратитесь к администратору.';
				|en = 'The report might contain incorrect data since migration to the new version is not completed. If the report is not available for a while, contact the administrator.'");
КонецФункции
			...
			ИначеЕсли Не ДоступенПоОпциям Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<Отчет ""%1"" отключен в настройках программы>';
						|en = '<Report %1 is disabled in settings>'"),
					ИмяОтчета);
			Иначе
