...
Процедура УстановитьПараметрСеансаСПаролем(Пароль) Экспорт
	ПараметрыСеанса.док_ПарольДляШифрования = Пароль;	
КонецПроцедуры
...
Функция Зашифровать(ТекстОригинальный) Экспорт
	...
	Хеш.Добавить(ПараметрыСеанса.док_ПарольДляШифрования);
...
Функция ПолучитьНастройкуШифрованияСправочника(Настройка) Экспорт
	...
	Запрос = Новый Запрос;	
	Запрос.УстановитьПараметр("Настройка", ?(ТипЗнч(Настройка) = Тип("Строка"),ПланыВидовХарактеристик.док_СправочникиДляШифрования[Настройка],Настройка));	
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	Значение
	|ИЗ
	|	РегистрСведений.док_СправочникиДляШифрования
	|
	|ГДЕ
	|	Настройка  = &Настройка";
	Выборка = Запрос.Выполнить().Выбрать();
	
	ПустоеЗначение = ?(ТипЗнч(Настройка) = Тип("Строка"),ПланыВидовХарактеристик.док_СправочникиДляШифрования[Настройка],Настройка).ТипЗначения.ПривестиЗначение();
