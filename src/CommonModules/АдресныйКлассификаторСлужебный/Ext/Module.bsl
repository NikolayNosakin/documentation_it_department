...
Функция ОписаниеПоследнейЗагрузки(ИдентификаторРегиона = Неопределено) Экспорт
	...
	ИначеЕсли ДнейРазницы > 2 Тогда
		Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Адресный классификатор был загружен %1 назад.';
																					|en = 'The address classifier was imported %1 ago.'"),
	...
	Иначе
		Если РезультатВебСервис.Данные.status = "PARTIALLY_RECOGNIZED" И ВСтруктуру Тогда
			...
			Адрес.comment = НСтр("ru = 'Нераспознанная часть адреса';
								|en = 'Unrecognized part of address'") + ": " + РезультатВебСервис.Данные.unrecognizedPart;
		Иначе
			Адрес = ОписаниеНовойКонтактнойИнформации();
			...
			Если Не ЕстьФайлВерсииСМуниципальнымиДанными Тогда
				...
				ТекстПричины = ?(ЕстьФайлВерсииСАдминистративнымиДанными,
					НСтр("ru = 'Не допускается загрузка адресных сведений без поддержки муниципальных адресов.';
						|en = 'Cannot import address data because municipal addresses are not supported.'"), 
					НСтр("ru = 'Файл не содержит адресных сведений';
						|en = 'The file does not contain address information'"));
				
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,
				,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось загрузить адресные сведения из файла: %1
				|по причине: %2
				|Используйте адресные сведения, расположенные по адресу https://its.1c.ru/download/fias2';
				|en = 'Cannot import address information from file:%1
				|due to: %2
				|Use address information from https://its.1c.ru/download/fias2'"), ПолноеИмяФайла, ТекстПричины));
...
Процедура СписокИзвлеченныхФайлов(КаталогФайлов)
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,
			,, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Список извлеченных файлов адресных сведений региона: %1';
																			|en = 'List of extracted files with address information for the region: %1'"), 
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		ИначеЕсли РегионЗагруженВАдресныеСведения.КодСубъектаРФ = Неопределено Тогда 
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Указан несуществующий регион ""%1""';
					|en = 'Non-existing ""%1"" state is specified'"), НаименованиеСубъектаРФ));
			Результат.АдресПроверен   = Истина;
		ИначеЕсли Не РегионЗагруженВАдресныеСведения.Загружен Тогда
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Отсутствуют адресные сведения по региону: %1';
																		|en = 'No address information for state %1'"),
	...
	Для Каждого КлючЗначение Из Адреса Цикл
		...
		Если НЕ ДомКорректный Тогда
			...
			Если ПохожиеВарианты.Количество() > 0 Тогда
				...
				Если ПохожиеВарианты.Количество() = 1 Тогда
					...
					ТекстПодсказки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'но есть %1';
																									|en = 'but there is %1'"), ПервыйВариант);
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 отсутствует в адресном классификаторе%2';
																		|en = '%1 is missing in the address classifier%2'"),
		...
		Если СтрСравнить(ПочтовыйИндекс, Адрес.ZipCode) <> 0 Тогда
			...
			ДополнительныйТекст = ?(ЗначениеЗаполнено(ПочтовыйИндекс), " " + НСтр("ru = 'Возможно, правильный индекс';
																					|en = 'Postal code might be correct'") + " "  + ПочтовыйИндекс, "");
			ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Индекс",
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Индекс ""%1"" не соответствует адресу.%2';
...
Процедура ПроверитьУчастки(Знач Участок, Знач ИндексАдреса, Знач ОшибкиПроверки, Знач Результат, Знач ТаблицаУчастков)
	...
	ДобавитьОшибкуПроверкиАдресаПоКлассификатору(ОшибкиПроверки, "Дом",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Земельный участок %1 отсутствует в адресном классификаторе';
																	|en = 'The %1 land lot is missing in the address classifier'"),
		Участок));
...
Функция ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(Знач АдресаДляПроверки, ЭтоУстановкаИдентификаторов = Истина)
	...
	Иначе
		...
		Иначе
			...
			|en = 'Address checking is unavailable due to:
			|- online user support is not connected
			|- there may be a problem of Internet service provider
	...
	Если ОписаниеФайла.УдалитьПутьПриЗакрытии Тогда
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,,, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Удаление файла региона %1';
																		|en = 'Delete region file %1'"), ОписаниеФайла.Путь));
...
Процедура ОбработатьОшибкуОбращенияКВебСервису(Результат, Знач Ответ, ИнформацияОбОшибке, ТекстURL)
	...
	Если ПустаяСтрока(ПредставлениеОшибки) Тогда
		...
		|en = 'Web service for working with addresses is unavailable due to:
		|- online user support is not connected
		|- there may be a problem of Internet service provider
		|- connection to the server is blocked by firewall, 
		| antivirus software, or another software
...
Функция КонтактнаяИнформацияВФорматеКлючЗначениеВJSON(Адрес)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Переданный адрес не может быть преобразован в функции %1.';
			|en = 'Function ""%1"" cannot convert the passed address.'"), "АдресныйКлассификаторСлужебный.КодыАдреса");
...
Функция ПолучитьФайлИзИнтернета(ОписаниеРегиона, Знач ВременныйКаталог, ПараметрыПолучения, ИмяФайла)
	...
	Если Не ФайлСуществует(ПутьДляСохранения) Тогда
		Результат.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			|en = 'Cannot import file %1. The address information import was terminated.
...
Процедура ЗагрузитьДанныеКлассификатораАдресовИзФайла(СубъектРФ, ОписаниеФайлов, ПараметрыЗагрузки) Экспорт
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,,, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка региона %1 из %2';
	...
	Попытка
		...
		ДлительныеОперации.СообщитьПрогресс(0,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загружается %1';
			...
			Исключение
				...
				ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка,
					...
					|due to: %3'"), СубъектРФ, НаименованиеРегиона, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
				ВызватьИсключение;
			КонецПопытки;
...
Процедура ЗагрузитьАдресныеОбъекты(КодСубъектаРФ, КаталогФайлов, ЗагружатьОднимНабором)
	...
	Если ФайлЗагрузки = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Загрузка адресных сведений прервана. В загружаемых файлах не найден файл с адресными сведениями.';
								|en = 'Address data import is canceled. File with address data was not found in imported files.'");
	КонецЕсли;
...
Процедура ОчиститьУровниСокращенийАдресныхСведений()
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Не удалось очистить регистр сведений Уровни сокращений адресных сведений.';
										|en = 'Cannot clear the Address data shortening levels information register.'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Примечание,,, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Чтение файла региона %1 из %2';
																	|en = 'Read a file of region %1 from %2'"), 
	...
	Если ТипXDTOЗаписи = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В файле ""%1"" не найден список записей в типе  %2';
																						|en = 'Record list in type %2 was not found in file ""%1""'"), Результат.ПолноеИмя, Результат.ТипXDTO);
	КонецЕсли;
	...
	Если НЕ КореньНайден Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В файле ""%1"" не найден корневой узел %2';
																						|en = 'Root node %2 is not found in file ""%1""'"), Результат.ПолноеИмя, ИмяОписывающегоУзла);
	КонецЕсли;
