...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	Если Не ПустаяСтрока(Минимальная)
		...
		ТекстСообщения = НСтр("ru = 'Минимальная версия платформы ""1С:Предприятие"" указана выше рекомендуемой в %1.
			...
			|en = 'The minimum 1C:Enterprise version is greater than the version recommended in %1.
			|The minimum version is ""%2"", the recommended version is ""%3"".'");
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ИдентификаторCOMСоединителя(Знач ИмяCOMСоединителя) Экспорт
	...
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'На задан CLSID для класса %1';
																					|en = 'The CLSID for class %1 is not specified.'"), ИмяCOMСоединителя);
...
Функция МинимальнаяВерсияПлатформы() Экспорт // АПК:581 - экспортная, так как используется при тестировании.
	...
	Возврат "8.3.20.1549; 8.3.21.1302; 8.3.22.1368";
