...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Проверка.Наименование                 = НСтр("ru = 'Исправление адресов содержащих устаревший тип здания Литер.';
												|en = 'Correcting addresses that contain the obsolete building type Liter.'");
	...
	Рекомендация                          = НСтр("ru = 'В зависимости от ситуации следует выбрать один из вариантов:
		...
		|•	Make sure that the address is entered correctly by checking it on the website of the Federal Tax Service of the Russian Federation:%1
		|If the address details on the website are incorrect or incomplete, contact the local authorities or the technical support of the Federal Tax Service.
		|•	If the address is outdated due to renaming, reassignment, or the abolition of address objects (streets, localities), then enter the current address.
		|•	If you cannot correct the address automatically, enter it manually by opening the address entry form.
		|
		|If the application uses the address classifier, do the following: 
		|•	First, update it to the latest version (in the Administration section).
		|•	To identify and automatically correct outdated addresses, import the change history for object addresses.
		|Quick access link: %2
		|
		|For distributed infobases (DIB), make changes only to the master node.
		|Then, sync the subordinate nodes.'");
		...
		Если ПроверенныйАдрес.Ошибки.Количество() > 0 Тогда
			УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = '%1 ""%2"" не соответствует адресному классификатору, по причине: %3';
					|en = '%1 %2 does not comply with the address classifier due to: %3'"), Строка(ОписаниеАдреса.Вид), ОписаниеАдреса.Представление, ПроверенныйАдрес.Ошибки[0].Текст);
		Иначе
			УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
