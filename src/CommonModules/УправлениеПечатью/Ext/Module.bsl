...
Процедура ОтключитьПользовательскийМакет(ПутьКМакету) Экспорт
	...
	Если ЧастиСтроки.Количество() <> 2 И ЧастиСтроки.Количество() <> 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Макет ""%1"" не существует.';
																						|en = 'Template ""%1"" does not exist.'"), ПутьКМакету);
	КонецЕсли;
	...
	Для Каждого ИмяМакета Из ИменаМакетов Цикл
		...
		Если Отказ Тогда
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru = 'При формировании печатной формы ""%1"" возникла ошибка. Обратитесь к администратору.';
				|en = 'Error occurred generating %1 print form. Contact your administrator.'"), ИмяМакета);
			ВызватьИсключение ТекстСообщенияОбОшибке;
		КонецЕсли;
...
Функция СкомпоноватьДанные(Параметры)
	...
	Если КлючевоеПоле = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не найдено ключевое поле ""Ссылка"" в списке полей схемы данных печати.';
								|en = 'The ""Reference"" key field is not found in the list of print data schema fields.'");
	КонецЕсли;
