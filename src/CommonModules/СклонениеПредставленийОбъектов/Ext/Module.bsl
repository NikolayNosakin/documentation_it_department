...
Процедура ДополнитьПредставлениеПроблемыРекомендацией(Представление)
	...
	Представление = Представление +	СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Для корректной работы склонения представлений рекомендуется:
			 ...
			 |en = 'For the correct operation of declining presentations, it is recommended that you:
			 |* either update the 1C:Enterprise technological platform to the %1 or more recent version,
			 |* or enable alternative declension service Morpher (in the Administration section).'"), 
		РекомендуемаяВерсия());
