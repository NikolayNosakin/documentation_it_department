	...
	Если Не ЗапускПрограммыЗавершен() Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимый вызов %1 при запуске программы.
			           ...
			           |The %2 procedure must be completed first.
			           |One of the event handlers might not have called the notification to continue.
			           |Last called procedure: %3.'"),
...
Процедура ПередЗавершениемРаботыСистемы(Отказ = Ложь, ТекстПредупреждения = "") Экспорт
	...
	Если Предупреждения.Количество() = 0 Тогда
		...
		ТекстПредупреждения = НСтр("ru = 'Завершить работу с программой?';
									|en = 'Do you want to exit the application?'");
		Отказ = Истина;
	Иначе
		Отказ = Истина;
...
Функция ИнформацияДляПоддержки() Экспорт
	...
	Текст = НСтр("ru = 'Название программы: [НазваниеПрограммы] 
	                   ...
	                   |en = 'Application name: [НазваниеПрограммы] 
	                   |Application version: [ВерсияПрограммы]
	                   |1C:Enterprise platform version: [ВерсияПлатформы][РазрядностьПлатформы]
	                   |Standard Subsystems Library version: [ВерсияБСП]
	                   |Operating system: [ОперационнаяСистема]
	                   |RAM: [ОперативнаяПамять]
	                   |COM connector: [ИмяCOMСоединителя]
	                   |Base configuration: [ЭтоБазоваяВерсияКонфигурации]
	                   |Full user: [ЭтоПолноправныйПользователь]
	                   |Training platform: [ЭтоУчебнаяПлатформа]
	                   |Configuration changed: [КонфигурацияИзменена]'") + Символы.ПС;
	...
	Если ПараметрыКлиента.ВыбратьНачальныеРегиональныеНастройкиИБ = Ложь Тогда
		...
		ПоказатьПредупреждение(
			...
			НСтр("ru = 'Вход в программу невозможен до установки начальных региональных настроек программы.
			           |Обратитесь к администратору за подробностями.';
			           |en = 'Cannot sign in because configuring the regional settings is required.
			           |Please contact the administrator.'"),
			15);
		Возврат;
	КонецЕсли;
