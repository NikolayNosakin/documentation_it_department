...
#КонецЕсли
	...
	ЗаписатьПримечание(Ссылка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Подключение, %1 = ""%2"".';
																	|en = 'Attachment, %1 = ""%2"".'"), "БезопасныйРежим", БезопасныйРежим));
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Элемент ""%1"" не подключен, потому что ""Дополнительные отчеты и обработки"" отключены в настройках программы.';
				|en = 'Cannot attach %1. Additional reports and data processors are disabled in program settings.'"),
			"'" + Строка(Ссылка) + "'");
		Возврат;
	КонецЕсли;
	...
	Если Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.Отчет
		...
		Исключение
			ПараметрыОтчета.ТекстОшибки = 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'При подключении дополнительного отчета ""%1"" возникла ошибка:';
																			|en = 'An error occurred while attaching additional report ""%1"":'"), Строка(Ссылка))
