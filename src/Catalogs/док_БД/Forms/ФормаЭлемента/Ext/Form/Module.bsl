...
Процедура СформироватьСтрокуСоединения()
	...
	Иначе	
		СтрокаСоединения = "Connect=Srvr=""" + Строка(Объект.Сервер) + """;Ref=""" + Объект.ИмяИБ + """";
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ТестоваяПриИзменении(Элемент)
	СрокЖизниВидимостьСервер();
КонецПроцедуры

&НаСервере
Процедура СрокЖизниВидимостьСервер()
	Элементы.СрокЖизни.Видимость = Объект.Тестовая;
	Объект.СрокЖизни = ?(Объект.Тестовая,Объект.СрокЖизни,Дата(1,1,1));
КонецПроцедуры
