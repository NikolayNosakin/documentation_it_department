...
Процедура ЗапуститьСозданиеНачальногоОбраза()
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Не удалось создать начальный образ по причине:';
								|en = 'Cannot create an initial image. Reason:'") + " " + Результат.КраткоеПредставлениеОшибки; 
	КонецЕсли;
...
Процедура СоздатьНачальныйОбразНаСервереЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Ошибка" Тогда
		ПроцентВыполнения = 0;
		Элементы.СтатусГотово.Заголовок = НСтр("ru = 'Не удалось создать начальный образ по причине:';
												|en = 'Cannot create an initial image. Reason:'") + " " + Результат.КраткоеПредставлениеОшибки;
		ЗапуститьПереходРезультат();
		Возврат;
	КонецЕсли;
