...
Процедура ЗавершениеПодписанияСохранитьВПапку(Результат, Контекст) Экспорт	
	...
	ПоказатьОповещениеПользователя(, , НСтр("ru = 'Подписание и сохранение завершено';
											|en = 'Signed and saved'"), БиблиотекаКартинок.Информация32);
КонецПроцедуры
...
Процедура ЗавершениеПодписанияФайлов(Результат, Контекст) Экспорт	
	...
	ПоказатьОповещениеПользователя(, , НСтр("ru = 'Сохранение и подписание завершено';
											|en = 'Saved and signed'"), БиблиотекаКартинок.Информация32);
	...
	Если ЗначениеЗаполнено(ИмяКаталога) Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Сохранение успешно завершено';
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'в папку %1';
																		|en = 'Folder: %1'"), ИмяКаталога), БиблиотекаКартинок.Информация32);
	КонецЕсли;
#КонецЕсли
