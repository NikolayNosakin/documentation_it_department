...
Процедура СохранитьВариант(Команда)
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Вариант отчета сохранен';
										|en = 'Report option is saved'"), Окно.ПолучитьНавигационнуюСсылку(), Заголовок);
...
Процедура СохранитьОтчет(Команда)
	...
	ТекстПредложения = НСтр("ru = 'Для сохранения результата отчета в файл рекомендуется
		...
		|en = 'To save a report result to a file, it is recommended that you
...
Процедура ПослеФормирования(Результат, ПараметрыФормирования) Экспорт 
	...
	Если Результат = Неопределено Тогда 
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Отчет не сформирован';
											|en = 'Report is not generated'"), Окно.ПолучитьНавигационнуюСсылку(), Заголовок);
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда 
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Отчет сформирован';
											|en = 'Report is generated'"), Окно.ПолучитьНавигационнуюСсылку(), Заголовок);
...
Процедура ПослеВыбораИмениФайла(Результат, Контекст) Экспорт 
	...
	Если Не ЗначениеЗаполнено(ПолноеИмяФайлаОтчета) Тогда
		...
		СписокФорматов.ПоказатьВыборЭлемента(ОписаниеОповещенияОЗакрытии, НСтр("ru = 'Выберите формат сохранения';
																				|en = 'Select a save format'"), ФорматПоУмолчанию);
	Иначе
