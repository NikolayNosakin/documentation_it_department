...
Процедура ЗаписатьТабличныйДокументИмяФайлаВыбрано(Знач ОбработчикЗавершения)
	...
	Если ПустаяСтрока(Параметры.ПутьКФайлу) Тогда
		...
		Иначе
			...
			ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Макет содержит ошибки. Продолжить запись?';
													|en = 'Template contains errors. Do you want to continue saving?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
		КонецЕсли;
	Иначе
