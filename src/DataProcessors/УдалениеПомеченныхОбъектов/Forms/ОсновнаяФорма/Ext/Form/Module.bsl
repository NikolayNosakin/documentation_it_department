...
Процедура УстановитьЗаменитьНа(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		Состояние(НСтр("ru = 'Для выбранной строки нельзя указать действие.';
						|en = 'Cannot specify an action for the selected row.'"),,,БиблиотекаКартинок.Предупреждение32);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьСнятьПометкуУдаленияПродолжение(Результат, СсылкиДляОбработки) Экспорт
	...
	Если ТипЗнч(Изменения) = Тип("Строка") Тогда
		Ошибка = НСтр("ru = 'Произошла ошибка при изменении пометки удаления по причине:';
						|en = 'An error occurred while changing the deletion mark due to:'")
					+ Символы.ПС + Изменения;
		ПоказатьПредупреждение(, Ошибка,,
			НСтр("ru = 'Ошибка при изменении пометки удаления.';
				|en = 'An error occurred while changing the deletion mark.'"));
	Иначе
	...
	ИначеЕсли ВыбранноеЗначение = "Удалить" И НЕ ТекущиеДанные.СсылочногоТипа Тогда
		Состояние(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Нельзя установить действие удалить для %1';
				|en = 'Cannot set the Delete action for %1'"),
