...
Процедура АдресныйКлассификаторУстарел(Команда)
	...
	СообщениеОПроверкеАдреса = НСтр("ru = 'Адресные сведения, загруженные в программу, устарели.
		...
		|en = 'The address details imported to the application are outdated.
		|Update the address classifier so that the address check is performed correctly.'");
	...
	Если КоличествоОшибок = 0 И ПредупреждатьОбОтсутствии Тогда
		...
		Если НЕ ВебСервисИспользуется И ТребуетсяОбновление Тогда
			...
			Иначе
				...
				|en = 'The address is entered correctly, but the address details imported to the application
				|have not been updated for a long time. It is recommended to update them and check the address again.
