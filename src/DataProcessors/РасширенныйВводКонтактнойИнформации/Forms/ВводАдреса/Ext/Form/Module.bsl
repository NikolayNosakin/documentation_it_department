...
Процедура АдресныйКлассификаторУстарел(Команда)
	...
	СообщениеОПроверкеАдреса = НСтр("ru = 'Адресные сведения, загруженные в программу, устарели.
		...
		|en = 'The address information imported to the application is outdated.
		|It is recommended that you update the address classifier so that the address check is performed correctly.'");
...
Процедура ОтобразитьПоляПоТипуАдреса()
	...
	Если ЭтоНациональныйАдрес Тогда
		...
		Элементы.НаселенныйПункт.ПодсказкаВвода      = НСтр("ru = 'Введите первые буквы названия населенного пункта';
															|en = 'Enter the first letters of locality name.'");
		Элементы.Улица.ПодсказкаВвода                = НСтр("ru = 'Введите первые буквы названия улицы';
															|en = 'Enter the first letters of street name.'");
	...
	Если КоличествоОшибок = 0 И ПредупреждатьОбОтсутствии Тогда
		...
		Если НЕ ВебСервисИспользуется И ТребуетсяОбновление Тогда
			Если ЕстьПравоЗагружатьКлассификатор Тогда
				...
				|en = 'Address is valid, but address classifier data was imported long ago.
				|It is recommended that you update the classifier and revalidate the address.'");
				ВопросОбОбновлениеАдресногоКлассификатора(СообщениеОПроверкеАдреса);
			Иначе
				...
				|en = 'The address is entered correctly, but the address information imported to the application
				|has not been updated for a long time. It is recommended to update it and check the address again.
				|To update the address classifier, contact the administrator.'");
				ПоказатьПредупреждение(, СообщениеОПроверкеАдреса);
			КонецЕсли;
		Иначе
