...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		...
		Иначе
			ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru = 'Не заданы параметры формы. Форма не может быть открыта.';
													|en = 'Cannot open the form. The form parameters are not specified.'"), Отказ);
			Возврат;
		КонецЕсли;
...
Функция Подключаемый_ЗапросПароляПользователя_ПриПереходеДалее(Отказ)
	...
	Если Не ПарольСинхронизацииЗадан
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не указан пароль.';
														|en = 'The password is blank.'"), , "WSПароль", , Отказ);
		Возврат Неопределено;
	КонецЕсли;
