	...
	Иначе
		Если Не ЭтоПолноправныйПользователь() Тогда
			ВызватьИсключение НСтр("ru = 'Для открытия обработки требуется право администрирования.';
									|en = 'To open a data processor, you must have the administration right.'")
		КонецЕсли;
...
Процедура ВыполнитьИзменениеПроверкаОтбораВыполнена(РезультатВопроса = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ЕстьНастроенныеИзменения() И Объект.ТипОперации = "ВвестиЗначения" Тогда
		ТекстВопроса = НСтр("ru = 'Изменения не настроены. Выполнить перезапись элементов без изменений?';
							|en = 'No changes found. Do you want to save the items without changes?'");
...
Процедура ВопросПерейтиКРазблокированиюРеквизитов(ВыбранныйРеквизит)
	...
	Если КонтекстныйВызов Тогда
		...
		ТекстВопроса = НСтр("ru = 'Реквизит заблокирован, перейти к разблокированию реквизитов?';
							|en = 'The attribute has been locked. Unlock attribute?'");
	Иначе
		...
		ТекстВопроса = ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для того чтобы не допустить рассогласования данных в программе,
				...
				|en = 'Attribute %1 is locked from editing
...
Процедура ИзменитьОбъекты()
	...
	Если Объект.ИзменятьВТранзакции Тогда
		...
		Если ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
			...
			ПоказатьОповещениеПользователя(НСтр("ru = 'Изменение выделенных элементов';
												|en = 'Edit selected items'"), ,НСтр("ru = 'Пожалуйста подождите, обработка может занять некоторое время...';
	...
	Иначе
		...
		Состояние(НСтр("ru = 'Обрабатываются элементы...';
						|en = 'Processing in progress…'"), 0, НСтр("ru = 'Изменение выделенных элементов';
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Изменение реквизитов элементов';
											|en = 'Edit attributes'"), , 
			ПодставитьПараметрыВСтроку(НСтр("ru = 'Изменены элементы (%1).';
	...
	Если Объект.ИзменятьВТранзакции И Не ОбработкаЗавершена Тогда
		...
		Если КоличествоПропущенных > 0 И Не ТекущееСостояниеИзменения.ПрерватьИзменение Тогда
			...
			СтрокаТаблицы.Объект = ПодставитьПараметрыВСтроку(НСтр("ru = '... и другие элементы (%1)';
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя)
	...
	ИначеЕсли ВРег(КлассОМ) = "РЕГИСТРРАСЧЕТА" Тогда
		...
		Иначе
			...
			Иначе
				ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru = 'Неизвестный тип объекта метаданных ""%1""';
																	|en = 'Invalid metadata object type: %1.'"), ПолноеИмя);
			КонецЕсли;
		КонецЕсли;
...
Функция ВидОбъектаПоТипу(Тип)
	...
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru = 'Неверный тип значения параметра (%1)';
															|en = 'Invalid parameter value type: %1.'"), Строка(Тип));
...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты)
	...
	ИначеЕсли ТипЗнч(Реквизиты) = Тип("Массив") Или ТипЗнч(Реквизиты) = Тип("ФиксированныйМассив") Тогда
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение ПодставитьПараметрыВСтроку(НСтр("ru = 'Неверный тип второго параметра Реквизиты: %1';
