...
Процедура ЗаполнитьБыстрыеОтборы(Знач ОписаниеОбработчиков = Неопределено)
	...
	ДобавитьБыстрыйОтбор(ОписаниеОбработчиков, МассивСтрок, "ИзмененаПроцедураПроверки", НСтр("ru = 'Изменена процедура проверки';
																								|en = 'Check procedure is changed'"), WebЦвета.Золотистый);
	...
	ДобавитьПрочийБыстрыйОтбор(ОписаниеОбработчиков, "ЗаписьЧитаемых", НСтр("ru = 'Запись читаемых';
																			|en = 'Writing of readable objects'"));
...
Функция ФильтрФайловРезервнойКопии()
	Возврат НСтр("ru = 'Описание обработчиков (*.handlers)|*.handlers|Все файлы (*.*)|*.*';
				|en = 'Handler details (*.handlers)|*.handlers|All files (*.*)|*.*'");
КонецФункции
...
Процедура УстановитьБыстрыйОтбор(ИмяБыстрогоОтбора)
	...
	ИначеЕсли ИмяБыстрогоОтбора = НСтр("ru = 'Отложенно';
										|en = 'Deferred'") Тогда
		Отбор = Новый Структура("РежимВыполнения", НСтр("ru = 'Отложенно';
														|en = 'Deferred'"));
		
	ИначеЕсли ИмяБыстрогоОтбора = НСтр("ru = 'Монопольно';
										|en = 'Exclusive'") Тогда
		Отбор = Новый Структура("РежимВыполнения", НСтр("ru = 'Монопольно';
														|en = 'Exclusive'"));
		
	ИначеЕсли ИмяБыстрогоОтбора = НСтр("ru = 'Оперативно';
										|en = 'Seamless'") Тогда
		Отбор = Новый Структура("РежимВыполнения", НСтр("ru = 'Оперативно';
														|en = 'Seamless'"));
		
	ИначеЕсли ИмяБыстрогоОтбора = НСтр("ru = 'Последовательно';
										|en = 'Sequentially'") Тогда
		Отбор = Новый Структура("РежимВыполненияОтложенныхОбработчиков", НСтр("ru = 'Последовательно';
																				|en = 'Sequentially'"));
		
	ИначеЕсли ИмяБыстрогоОтбора = НСтр("ru = 'Параллельно';
										|en = 'Parallel'") Тогда
		Отбор = Новый Структура("РежимВыполненияОтложенныхОбработчиков", НСтр("ru = 'Параллельно';
																				|en = 'Parallel'"));
		
	ИначеЕсли СтрНайти(ИмяБыстрогоОтбора, НСтр("ru = 'Начальное заполнение';
												|en = 'Initial filling'")) > 0 Тогда
	...
	ИначеЕсли СтрНайти(ИмяБыстрогоОтбора, НСтр("ru = 'Обязательный';
												|en = 'Required'")) > 0 Тогда
		Отбор = Новый Структура("ВыполнятьВГруппеОбязательных", Истина);
		
	ИначеЕсли СтрНайти(ИмяБыстрогоОтбора, НСтр("ru = 'Многопоточный';
												|en = 'Multi-threaded'")) > 0 Тогда
		Отбор = Новый Структура("Многопоточный", Истина);
		
	ИначеЕсли СтрНайти(ИмяБыстрогоОтбора, НСтр("ru = 'Запись читаемых';
												|en = 'Writing of readable objects'")) > 0 Тогда
		Отбор = Новый Структура("ЗаписьЧитаемых", Истина);
		
	ИначеЕсли СтрНайти(ИмяБыстрогоОтбора, НСтр("ru = 'Повторная запись';
												|en = 'Rewriting'")) > 0 Тогда
		Отбор = Новый Структура("ПовторнаяЗапись", Истина);
		
	ИначеЕсли СтрНайти(ИмяБыстрогоОтбора, НСтр("ru = 'Тех.проект';
												|en = 'Technical design'")) > 0 Тогда
		Отбор = Новый Структура("ТехническийПроект", Истина);
