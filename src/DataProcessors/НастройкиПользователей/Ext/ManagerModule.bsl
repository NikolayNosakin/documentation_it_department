...
Функция НастройкиВсехФорм(ИмяПользователя)
	...
	СписокФорм.Добавить("ВнешняяОбработка.StandardEventLog.Форма.EventsJournal", 
		НСтр("ru = 'Стандартные.Журнал регистрации';
			|en = 'Standard.Event Log'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardEventLog.Форма.EventForm", 
		НСтр("ru = 'Стандартные.Журнал регистрации, Событие';
			|en = 'Standard.Event Log, Event'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardEventLog.Форма.EventsJournalFilter", 
		НСтр("ru = 'Стандартные.Журнал регистрации, Настройка отбора событий';
			|en = 'Standard.Event Log, Event filter settings'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardFindByRef.Форма.MainForm", 
		НСтр("ru = 'Стандартные.Поиск ссылок на объект';
			|en = 'Standard.Find references to objects'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardFullTextSearchManagement.Форма.MainForm", 
		НСтр("ru = 'Стандартные.Управление полнотекстовым поиском';
			|en = 'Standard.Manage Full Text Search'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardDocumentsPosting.Форма.MainForm", 
		НСтр("ru = 'Стандартные.Проведение документов';
			|en = 'Standard.Document Posting'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardDeleteMarkedObjects.Форма.Form", 
		НСтр("ru = 'Стандартные.Удаление помеченных объектов';
			|en = 'Standard.Delete Marked Objects'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardExternalDataSourceManagement.Форма.Form", 
		НСтр("ru = 'Стандартные.Управление внешними источниками данных';
			|en = 'Standard.Management of external data sources'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardTotalsManagement.Форма.MainForm", 
		НСтр("ru = 'Стандартные.Управление итогами';
			|en = 'Standard.Totals management'") , Ложь, БиблиотекаКартинок.Форма);
	СписокФорм.Добавить("ВнешняяОбработка.StandardActiveUsers.Форма.ActiveUsersListForm", 
		НСтр("ru = 'Стандартные.Активные пользователи';
			|en = 'Standard.Active users'") , Ложь, БиблиотекаКартинок.Форма);
...
Процедура ЗаполнитьСписокНастроекВнешнегоВида(Параметры)
	...
	Для Каждого НастройкаФормы Из НастройкиФорм Цикл
		...
		ИначеЕсли ПредставлениеОбъектаМетаданных[0] = НСтр("ru = 'Стандартные';
															|en = 'Standard'") Тогда
...
Процедура ЗаполнитьСписокПрочихНастроек(Параметры)
	...
	Если Ключи.Количество() > 0 Тогда
		...
		Если Не ПустаяСтрока(Параметры.Поиск) Тогда
			Если СтрНайти(ВРег(НСтр("ru = 'Персональные настройки';
									|en = 'Personal settings'")), ВРег(Параметры.Поиск)) = 0 Тогда
	...
	Если Не ПустаяСтрока(Параметры.Поиск) Тогда
		...
		Если СтрНайти(ВРег(НСтр("ru = 'Настройки печати табличных документов';
								|en = 'Spreadsheet document print settings'")), ВРег(Параметры.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
	...
	Если Не ПустаяСтрока(Параметры.Поиск) Тогда
		...
		Если СтрНайти(ВРег(НСтр("ru = 'Избранное';
								|en = 'Favorites'")), ВРег(Параметры.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
	...
	Если Не ПустаяСтрока(Параметры.Поиск) Тогда
		...
		Если СтрНайти(ВРег(НСтр("ru = 'Прочие настройки';
								|en = 'Other settings'")), ВРег(Параметры.Поиск)) = 0 Тогда
			ВыводитьНастройку = Ложь;
		КонецЕсли;
...
Процедура ДобавитьНастройкиРабочегоСтолаИКомандногоИнтерфейса(Параметры, ДеревоНастроек)
	...
	Если Не ПустаяСтрока(Параметры.Поиск) Тогда
		Если СтрНайти(ВРег(НСтр("ru = 'Командный интерфейс и начальная страница';
								|en = 'Command interface and home page'")), ВРег(Параметры.Поиск)) = 0 Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
